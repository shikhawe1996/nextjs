...
const MultiCard = () => {
  ...
  const filterData = MultiData.filter((item) => (
  ...
  return (
    <section className="multiple-cards ">
      <div className="container ">
        <div className="button-wrap flex gap-1.5 justify-end mb-[30px]">
          <label htmlFor="search">Search:</label>
          <input className=" bg-white text-black border-orange  focus:outline-none focus:ring-2 focus:ring-orange" type="text"  value={search} onChange={handleSearch} />
        </div>
        ...
        <div className="multiple flex w-[calc(100%+20px)] mx-[-10px] flex-wrap mt-[30px] xxs:max-md:w-full xxs:max-md:flex-col">
          ...
          {filterData.map((item, index) => (
            <div
              key={index}
              className="cards hover:scale-105 transition-transform duration-300 ease-in-out w-[calc(25%-20px)] mb-[25px] mx-[10px] p-[20px] bg-white border-solid border-stone-400 border rounded-lg xxs:max-md:w-full xxs:max-md:mb-[20px]"
            >
